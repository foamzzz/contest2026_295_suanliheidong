...
static int rv_guard_alloc(struct rv_guard_buf_s *buf, size_t cap)
{
  ...
  *(uint32_t *)buf->raw = RV_ASR_GUARD;
  *(uint32_t *)(buf->raw + 4 + cap) = RV_ASR_GUARD;
  buf->data = buf->raw + 4;
  buf->cap = cap;
  return 0;
}
...
{
  return buf != NULL && buf->raw != NULL && buf->data != NULL &&
         buf->cap != 0 && *(const uint32_t *)buf->raw == RV_ASR_GUARD &&
         *(const uint32_t *)(buf->data + buf->cap) == RV_ASR_GUARD;
}
