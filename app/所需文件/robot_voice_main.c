  ...
  while (!g_voice_stop_requested)
    {
      ...
      if (voice_get_api_key(key, sizeof(key)) < 0)
        {
      ...
      printf("[RV-VOICE] invoking ASR bytes=%zu\n", pcm_len);
      printf("[RV-ASR] request begin\n");
      ret = mimo_asr_recognize(key, pcm, pcm_len, text, sizeof(text));
...
static int voice_channel_start_mode(int oneshot)
{
  ...
  contest_voice_set_state(CONTEST_VOICE_RECORDING);
  if (pthread_create(&g_voice_thread, NULL, voice_record_worker, NULL) != 0)
    {
      g_voice_running = 0;
      g_voice_oneshot = 0;
      g_voice_worker_exited = true;
...
int voice_channel_test_asr(const char *pcm_path)
{
  ...
  close(fd);
  ret = mimo_asr_recognize(key, pcm, len, text, sizeof(text));
  free(pcm);
  if (ret == 0)
    {
...
static int robot_voice_asr_test(void)
{
  ...
  ret = robot_voice_capture_record(pcm, cap, &len,
                                   ROBOT_VOICE_CAPTURE_SECONDS * 1000);
  if (ret == 0) ret = mimo_asr_recognize(g_api_key, pcm, len, text, sizeof(text));
