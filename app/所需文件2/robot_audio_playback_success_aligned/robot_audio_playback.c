...
/* Keep the successful network-isolated behavior: because the current MiMo
 * transport buffers the complete HTTP response before PCM decoding, normal
 * utterances begin playback at EOS.  Very long utterances start when this ring
 * fills so the producer never deadlocks.
 */
  ...
  while (done < len)
    {
      ...
      if (pb->ring.count == RV_RING_BYTES)
        {
          rv_unlock(pb);
      ...
      memcpy(pb->ring.data + pb->ring.head, src + done, first);
      ...
      if (first < n)
        {
          ...
          memcpy(pb->ring.data + pb->ring.head, src + done, second);
...
int robot_audio_playback_open(void)
{
  ...
  /* Exact order from the known-good implementation: channels first, then
   * sample rate, then data width.  Rate calculation in the lower half depends
   * on the active slot format, so do not reorder these calls.
   */
  ...
  printf("[RV-PB] I2S configured source=%uHz/mono/%ubit "
         "wire=%uHz/%uch/%ubit source_block=%u wire_dma=%u "
         "frame_ms=21.33\n",
  ...
  printf("[RV-PB] open ring=%u prebuffer=%u rebuffer=%u "
         "worker_stack=%u worker_prio=%u dma_slots=%u\n",
         RV_RING_BYTES, RV_START_PREBUFFER_BYTES, RV_REBUFFER_BYTES,
         RV_WORKER_STACK, RV_WORKER_PRIORITY, RV_MAX_INFLIGHT);
  return 0;
...
int robot_audio_playback_finish(void)
{
  ...
  printf("[RV-PB] finish total_pcm=%zu result=%d\n",
         pb->total_written, ret);
  return ret;
}
...
void robot_audio_playback_close(void)
{
  ...
  printf("[RV-PB] close total_pcm=%zu underruns=%lu result=%d "
         "slots=%lu/%lu,%lu/%lu\n",
         pb->total_written,
